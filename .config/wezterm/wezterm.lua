...
-- Theme
config.colors = wezterm.gui.get_appearance():find('Dark') and require('themes.blight') or require('themes.ornament')
...
local function calculate_gradient(colors, total_tabs)
    local gradient_to, gradient_from = wezterm.color.parse(colors.tab_bar.active_tab.bg_color)
    gradient_from = wezterm.gui.get_appearance():find('Dark') and gradient_to:lighten(0.1) or gradient_to:darken(0.1)
    return wezterm.color.gradient({ orientation = 'Horizontal', colors = { gradient_from, gradient_to } }, total_tabs)
end

wezterm.on('format-tab-title', function(tab, tabs, panes, config, hover, max_width)
    local colors = config.resolved_palette
    local gradient = calculate_gradient(colors, #tabs)

    local is_next_tab_active = tabs[tab.tab_index + 2] and tabs[tab.tab_index + 2].is_active
    local background_color = tab.is_active and gradient[#tabs - tab.tab_index] or colors.tab_bar.background
    local next_background_color = is_next_tab_active and gradient[#tabs - tab.tab_index - 1]
        or colors.tab_bar.background
...
wezterm.on('update-right-status', function(window, pane)
    ...
    local gradient = calculate_gradient(colors, #segments)

    local elements = {}
    for index, segment in ipairs(segments) do
        if index == 1 then table.insert(elements, { Background = { Color = colors.tab_bar.background } }) end
...
config.keys = {
    ...
    -- Launcher
    {
        key = 'Space',
        mods = 'LEADER',
        action = wezterm.action.ShowLauncherArgs({ flags = 'FUZZY|DOMAINS|COMMANDS', fuzzy_help_text = '> ' }),
    },
    ...
    -- Workspace
    {
        key = 'w',
        mods = 'LEADER',
        action = wezterm.action.ShowLauncherArgs({ flags = 'WORKSPACES', help_text = 'Select a workspace' }),
    },
    ...
    copy_mode = {
        {
            key = 'Escape',
            mods = 'NONE',
            action = wezterm.action.Multiple({ 'ScrollToBottom', { CopyMode = 'Close' } }),
        },
